...
def test_export_metadata():
    ...
    # Upload, Process, Detect
    upload_resp = client.post("/upload-pdf", files={"file": ("export_diagram.pdf", pdf_bytes, "application/pdf")})
    doc_id = upload_resp.json()["document_id"]
    client.post(f"/documents/{doc_id}/process")
    client.post(f"/documents/{doc_id}/detect-shapes")
    ...
    assert "shape_id" in export_data[0]
    assert "bbox" in export_data[0]
    assert "properties" in export_data[0]
    assert "svg_path" in export_data[0]
    assert "Content-Disposition" in export_resp.headers
    assert f"document_{doc_id}_metadata.json" in export_resp.headers["Content-Disposition"]
